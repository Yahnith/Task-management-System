...
const addTask = async (req, res) => {
    ...
    try {
        ...
        if (task.length < 10) {
            return res.status(400).send('add minimum 10 char');
        }

        const taskDetail = await new Task({ task, cretedBy: id });
        await taskDetail.save();
        return res.status(200).send(taskDetail);
    } catch (error) {
        // Consider logging or sending a more informative error message
